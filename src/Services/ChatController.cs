...
public class ChatController
{
    ...
    private void EnsureServicesInitialized()
    {
        ...
        if (_claudeApi != null && _openAiApi != null && model == _lastInitializedModel) return;

        _claudeApi = new ClaudeApiService(_httpClientService, _configService, _logService);
        _openAiApi = new OpenAIService(_httpClientService, _configService, _logService);
    ...
    private async Task<ApiResponse> SendAiRequestAsync(
        ...
        Action<int, int, int>? onRetryWait = null)
    {
        ...
        {
            return await _openAiApi!.SendRequestAsync(
                systemPrompt,
                messages,
                tools,
                onTextDelta,
                (toolName, _) => onToolStart(toolName),
                onRetryWait);
        }

        return await _claudeApi!.SendStreamingRequestAsync(
            systemPrompt,
            messages,
            tools,
            onTextDelta,
    ...
    private async void HandleChatMessage(JsonObject? data)
    {
        ...
        try
        {
            ...
            DiagLog($"[4/6] Sending to {_configService.GetConfig().Provider} API (mode: {mode}, messages: {messages.Count}, tools: {tools?.Count ?? 0}, context: {(_cachedContext != null ? "loaded" : "none")}, caching: {cachingEnabled})");
            ...
            for (var round = 0; round < maxToolRounds; round++)
            {
                ...
                var response = await SendAiRequestAsync(
                    systemPrompt,
                    messages,
                    tools,
                    token => SendToWebView("chat_streaming", new { token, done = false }),
                    toolName => SendToWebView("tool_start", new { toolName }),
                    (attempt, delaySec, maxRetries) => SendToWebView("retry_wait", new { attempt, delaySec, maxRetries }));
    ...
    private void HandleGetSettings()
    {
        ...
        SendToWebView("load_settings", new
        {
            ...
            hasKey = _configService.HasApiKey(config.Provider),
            hasClaudeKey = _configService.HasApiKey("claude"),
            hasOpenAiKey = _configService.HasApiKey("openai"),
    ...
    private void HandleSaveSettings(JsonObject? data)
    {
        ...
        var provider = data?["provider"]?.GetValue<string>();
        ...
        _configService.SaveConfig(apiKey, provider, model, depth, tokens, theme, retryMaxAttempts, retryDelaySeconds, maxToolRounds, promptCachingEnabled, autoRefreshContext, autoLoadLastConversation);
